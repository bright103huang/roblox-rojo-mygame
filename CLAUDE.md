...
AlchemyUI.legacy.lua  → 药材选择面板 + 配方提示 + 炼制结果弹窗
...
| `Client/AlchemyUI.legacy.lua` | 炼丹面板：药材选择、配方提示、结果弹窗 |
...
### 客户端脚本 RunContext
所有客户端 UI 脚本（StatusUI、DayNightUI、AlchemyUI、ShopUI、ChaosEventUI）使用 `.legacy.lua` 后缀，在 Studio 中 RunContext 设为 Legacy。不要在代码中通过 `pcall` 设置 RunContext，无效。
