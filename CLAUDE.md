...
本游戏使用 8 个专业化 AI 子代理（仙官司）协作开发：

| 仙官司 | Agent ID | 职责 |
|--------|----------|------|
| 灵感司 | `inspiration` | 需求挖掘与方案设计（动工前探讨） |
| 营造司 | `architect` | 游戏架构设计与全局规划 |
| 界面司 | `ui-artisan` | 创建和修改客户端 UI，纯 Luau 代码 |
| 任务司 | `task-craftsman` | 按 Task Handler 模式创建任务逻辑 |
| 查察司 | `qa-inspector` | 代码审查、架构一致性 |
| 诊察司 | `diagnostician` | 系统化 Bug 定位与根因修复 |
| 数据司 | `data-steward` | Config 配置表、DataManager 数据层 |
| 督工司 | `overseer` | 子代理协调与任务派发，统筹全局 |

协作流程：**灵感司探需求 → 营造司做设计 → 数据司定 Config → 界面司/任务司并行实施 → 查察司审查 → 诊察司修 Bug（需要时）**
