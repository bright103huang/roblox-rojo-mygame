...
开始工作前调用 `roblox-game-development` + `systematic-debugging` skill。

## 审查清单

### 1. 文件后缀与 RunContext
- `.local.lua` = LocalScript（Legacy），独立运行
- `.lua` = ModuleScript，被 require
- `.server.lua` = Script，服务端
- 混用会导致脚本不执行或重复执行
